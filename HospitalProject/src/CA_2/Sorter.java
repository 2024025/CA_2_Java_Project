...
public class Sorter { //Declared as public to be acessible in other classes
    ...
    public void mergeSortList(List<Person> people, int left, int right) {
        ...
        if (left < right) {
            ...
            mergeSortList(people, left, right); //Recursive call responsible to divide the list from left part up to middle.
            mergeSortList(people, middle + 1, right);//Recursive call responsible to divide the list from middle +1 to the right
            ...
            mergeSortedSubLists(people, left, middle, right);
            ...
            if (left == 0 && right == people.size() - 1) {
                System.out.println("\nSorted List: "); //Message to interact with the user
                for (Person person : people) { //For each loop iterating each element from people List.
    ...
    private void mergeSortedSubLists(List<Person> list, int left, int middle, int right) {
        ...
        int k = 0;// index that will be used to place the elements back to the original list.
        ...
        while (i < leftList.size()) {
            list.set(k++, leftList.get(i++));//Adds the current element to the leftList and increments i and k
        }
        //Loop used to add remaing elements from the rightList to the original list
        while (j < rightList.size()) {
            list.set(k++, rightList.get(j++));//Adds the current element to the rightList and increments i and k
        }
    }
}
